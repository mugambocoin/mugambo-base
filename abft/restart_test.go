...
func testRestart(t *testing.T, weights []pos.Weight, cheatersCount int) {
	...
	lchs := make([]*TestMugamboBFT, 0, COUNT)
	...
	for _, _lch := range lchs {
		lch := _lch // capture
		lch.applyBlock = func(block *mugambobft.Block) *pos.Validators {
	...
	for n, e := range ordered {
		if r.Intn(10) == 0 || n%20 == 0 {
			...
			restored := NewIndexedMugamboBFT(store, prev.input, &adapters.VectorToDagIndexer{vecfc.NewIndex(prev.crit, vecfc.LiteConfig())}, prev.crit, prev.config)
			assertar.NoError(restored.Bootstrap(prev.callback))

			lchs[RESTORED].IndexedMugamboBFT = restored
		}
...
func compareStates(assertar *assert.Assertions, expected, restored *TestMugamboBFT) {
	assertar.Equal(
...
func compareBlocks(assertar *assert.Assertions, expected, restored *TestMugamboBFT) {
